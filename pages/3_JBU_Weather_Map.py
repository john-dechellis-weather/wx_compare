...
# FLEET SWEEP, ROUTES AND TRACKS live in core/fleet.py (21 Sep).
# They moved so the login map can draw the fleet and so a background
# warmer (started from Homepage) keeps sweeping with nobody on this
# page. Everything below that used to own that state now reads the
# same objects from core.fleet through thin wrappers.
from core import fleet as _CF
...
def _note_positions(rows):
    """Now core.fleet.note_positions; the sweep calls it itself."""
    _CF.note_positions(rows)


def _trail_paths(max_age_s: float = 1800.0, steps: int = 8):
    """Smoothed trail per aircraft - core.fleet.trail_paths."""
    return _CF.trail_paths(max_age_s=max_age_s, steps=steps)
...
def _hold_candidates(dest_by_cs: dict = None,
    ...
    out = []
    # Snapshot under the lock: the fleet warmer appends to these from
    # its own thread.
    with _CF.TRACKS_LOCK:
        _items = [(c, list(v)) for c, v in _TRACKS.items()]
    for cs, h in _items:
        if len(h) < HOLD_FIXES:
            continue
...
# Sweep cadence: core.fleet owns it (JBU_FLEET_SWEEP_S).
FLEET_SWEEP_S = _CF.FLEET_SWEEP_S
...
def _kick_fleet_now():
    """Start a sweep at module load if this bucket has none yet."""
    _CF.kick(_fleet_bucket())


def fleet_now(bucket: str):
    """(result, err, tb) - the last good sweep, at once."""
    return _CF.fleet_now(bucket, FIRST_WAIT_S)


def cached_fleet(bucket: str):
    """Kept for callers by name. The sweep is core.fleet.sweep(), run
    once per bucket by the fleet warmer; this returns its result."""
    _CF.refresh(bucket)
    return _CF.STATE["res"]
...
if run_button or _auto:
    ...
    with st.spinner(f"Fetching TAFs for {len(JETBLUE_ICAOS)} stations..."):
        try:
            ...
            # The key carries the window LENGTH, not its start. With
            # the start hour in the key, every top of the hour was a
            # new key with nothing cached, and whoever loaded the page
            # first waited for the full TAF fetch and parse. Now the
            # previous hour's result is shown at once and the new
            # window is computed in the background: the stored window
            # start differing from this run's forces that refresh
            # (ttl 0) even when the entry is young.
            _akey = (f"analyze|{hours_ahead}h|"
                     f"{vis_threshold}|{ceiling_threshold}|{tsra_enabled}")
            _swin = _swr_state()
            _win_changed = _swin.get(_akey + "|win") != now.isoformat()
            _swin[_akey + "|win"] = now.isoformat()
            results, _aerr = _swr(
                _akey,
                ...
                ttl_s=0.0 if _win_changed else 600.0)
            if results is None:
