...
if show_ac:
    ...
    if _trail:
        ...
        if scope:
            # HISTORY DOTS, the STARS idiom: the last few positions,
            # fading, in the operator's colour. Heading and speed read
            # off the spacing without a line through the map.
            _dots = []
            _snap = _TK.snapshot()
            _nhist = 6
            for _d in _trail:
                _h = _snap.get(_d["hex"]) or []
                _h = _h[-_nhist - 1:-1]          # the fixes BEFORE the current one
                for _i, (_lo, _la, _ts) in enumerate(_h):
                    _a = int(60 + 150 * (_i + 1) / max(1, len(_h)))
                    _dots.append({"lon": _lo, "lat": _la,
                                  "col": _d["color"][:3] + [_a]})
            if _dots:
                layers.append(pdk.Layer(
                    "ScatterplotLayer", data=_dots, get_position="[lon, lat]",
                    get_fill_color="col", get_radius=1,
                    radius_min_pixels=1.6, radius_max_pixels=1.6,
                    pickable=False))
        else:
            layers.append(pdk.Layer(
