"""Archive Radar Position — WITH DEBUG PRINTS for troubleshooting."""
from __future__ import annotations

from datetime import datetime, timezone, timedelta

import streamlit as st

st.set_page_config(
    page_title="BlueMet — Archive Radar Position",
    layout="wide",
)

from retro_theme import apply_retro_theme
apply_retro_theme()

from auth import check_password
check_password()


@st.cache_data(ttl=86400, show_spinner=False, max_entries=10)
def cached_render(
    request_time_iso: str,
    aircraft_lat: float,
    aircraft_lon: float,
    callsign: str,
    station: str,
    zoom_deg: float,
) -> tuple[bytes, bytes, str, str]:
    print(f"[CACHE DEBUG] cached_render called for station={station}")
    from core.radar import fetch_and_render_radar

    request_time = datetime.fromisoformat(request_time_iso)
    return fetch_and_render_radar(
        target_time=request_time,
        aircraft_lat=aircraft_lat,
        aircraft_lon=aircraft_lon,
        callsign=callsign,
        station=station,
        zoom_deg=zoom_deg,
    )


st.title("Archive Radar Position")
st.caption("Plot aircraft position on archived NEXRAD Level III radar imagery.")

with st.sidebar:
    st.header("Date & Time (UTC)")
    date_input = st.date_input(
        "Date",
        value=datetime(2026, 7, 6).date(),
        min_value=datetime(2022, 1, 1).date(),
        max_value=datetime.now(timezone.utc).date(),
    )
    time_input = st.time_input(
        "Time (UTC)",
        value=datetime(2026, 7, 6, 23, 30).time(),
        step=timedelta(minutes=5),
    )

    st.divider()
    st.header("Aircraft Position")
    lat_input = st.text_input("Latitude", value="39.76")
    lon_input = st.text_input("Longitude", value="-86.15")
    callsign_input = st.text_input("Callsign / Label", value="TEST")

    st.divider()
    st.header("Radar")
    station_input = st.text_input("Select Radar Site", value="KFTG", max_chars=4).strip().upper()
    zoom_input = st.slider("Zoom (degrees)", 0.5, 5.0, 2.0, 0.5)

    st.divider()
    run_button = st.button("Fetch & Render", type="primary", use_container_width=True)


if run_button:
    st.write("🔵 DEBUG: Button was clicked")
    print("[PAGE DEBUG] Button clicked, starting validation")

    errors = []
    if not station_input or len(station_input) not in (3, 4):
        errors.append("Radar site must be 3 or 4 characters.")
    if not lat_input.strip():
        errors.append("Please enter a latitude.")
    if not lon_input.strip():
        errors.append("Please enter a longitude.")

    aircraft_lat = None
    aircraft_lon = None
    if lat_input.strip():
        try:
            aircraft_lat = float(lat_input)
        except ValueError:
            errors.append("Latitude must be a number.")

    if lon_input.strip():
        try:
            aircraft_lon = float(lon_input)
        except ValueError:
            errors.append("Longitude must be a number.")

    if errors:
        for e in errors:
            st.error(e)
        st.stop()

    st.write("🔵 DEBUG: Validation passed")
    print("[PAGE DEBUG] Validation passed")

    request_time = datetime.combine(date_input, time_input, tzinfo=timezone.utc)

    if request_time > datetime.now(timezone.utc):
        st.error("Requested time is in the future.")
        st.stop()

    if len(station_input) == 4 and station_input.startswith("K"):
        station_code = station_input[1:]
    else:
        station_code = station_input

    callsign = callsign_input.strip() or "AIRCRAFT"

    st.write(f"🔵 DEBUG: About to call cached_render with station={station_code}")
    print(f"[PAGE DEBUG] About to call cached_render with station={station_code}")

    try:
        refl_png, vel_png, refl_time, vel_time = cached_render(
            request_time_iso=request_time.isoformat(),
            aircraft_lat=aircraft_lat,
            aircraft_lon=aircraft_lon,
            callsign=callsign,
            station=station_code,
            zoom_deg=zoom_input,
        )
        st.write(f"🔵 DEBUG: cached_render returned. refl_png bytes={len(refl_png)}, vel_png bytes={len(vel_png)}")
        print(f"[PAGE DEBUG] cached_render returned. refl_png bytes={len(refl_png)}, vel_png bytes={len(vel_png)}")
    except Exception as e:
        st.error(f"cached_render failed: {e}")
        import traceback
        st.code(traceback.format_exc())
        st.stop()

    st.subheader("Base Reflectivity (N0B)")
    st.caption(f"Dataset: `{refl_time}`")
    if refl_png:
        st.image(refl_png, use_container_width=True)
    else:
        st.error("Reflectivity PNG is empty!")

    st.subheader("Base Velocity")
    st.caption(f"Dataset: `{vel_time}`")
    if vel_png:
        st.image(vel_png, use_container_width=True)
    else:
        st.warning("Velocity product not available.")

else:
    st.write("Click Fetch & Render to test.")
